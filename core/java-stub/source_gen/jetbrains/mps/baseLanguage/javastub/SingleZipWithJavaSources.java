...
public class SingleZipWithJavaSources implements JavadocSupplier {
  ...
  public void acquire() {
    ...
    try {
      ...
      try {
        ...
        if (LOG.isInfoLevel()) {
          LOG.info(String.format("%s failed", SingleZipWithJavaSources.class.getSimpleName()), ex);
        }
      }
    } finally {
      myZipGuard.release();
  ...
  public Documentation apply(ASMClass aClass) {
    ...
    try {
      ...
      if (LOG.isDebugLevel()) {
        LOG.debug(String.format("Failed to read/parse source for %s from %s", entry.getName(), myZipFile.getName()), ex);
      }
      return null;
    } finally {
