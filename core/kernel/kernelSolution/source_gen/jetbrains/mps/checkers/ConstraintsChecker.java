  ...
  public void checkNodeInEditor(final SNode node, LanguageErrorsCollector errorsCollector, SRepository repository) {
    ...
    if (parent != null) {
      ...
      if (SNodeOperations.getConcept(parent).isValid()) {
        ...
        final CheckingNodeContextImpl context = new CheckingNodeContextImpl();
        boolean success = errorsCollector.runCheckingAction(() -> ModelConstraints.canBeChild(node, context));
        if (!(success)) {
          SNodeReference breakingNode = context.getBreakingNode();
          TypesystemRuleId ruleId = breakingNode == null ? null : new TypesystemRuleId(breakingNode);
          String message = ModelConstraints.
