...
public class KtClassParser {
  public static SNode parseClass(final KmClass kmClass, final SNode node, final List<SNode> childClasses, final List<SNode> enumEntries, final KtReadContext context) {
    final String fqName = kmClass.getName();
    {
      ...
      if (SNodeOperations.isInstanceOf(visible, CONCEPTS.IVisible$LZ)) {
        SLinkOperations.setTarget(visible, LINKS.visibility$vnSV, SConceptOperations.createNewNode(SNodeOperations.asInstanceConcept(KtAttributes.getVisibility(Attributes.getVisibility(kmClass)))));
      }
    }

    {
      final SNode iClass = node;
      if (SNodeOperations.isInstanceOf(iClass, CONCEPTS.IClassDeclaration$bQ)) {
        SLinkOperations.setTarget(iClass, LINKS.inheritance$TFvr, SConceptOperations.createNewNode(SNodeOperations.asInstanceConcept(KtAttributes.getModality(Attributes.getModality(kmClass)))));

        {
          final SNode actualClass = node;
          if (SNodeOperations.isInstanceOf(actualClass, CONCEPTS.ClassDeclaration$Jm)) {
            if (Attributes.isInner(kmClass)) {
              SLinkOperations.setNewChild(actualClass, LINKS.modifier$C$4W, CONCEPTS.InnerClassModifier$wL);
            } else if (Attributes.isData(kmClass)) {
              SLinkOperations.setNewChild(actualClass, LINKS.modifier$C$4W, CONCEPTS.DataClassModifier$wi);
            } else if (Attributes.isValue(kmClass)) {
              SLinkOperations.setNewChild(actualClass, LINKS.modifier$C$4W, CONCEPTS.ValueClassModifier$84);
            } else if (Objects.equals(Attributes.getKind(kmClass), ClassKind.ANNOTATION_CLASS)) {
              SLinkOperations.setNewChild(actualClass, LINKS.modifier$C$4W, CONCEPTS.AnnotationClassModifier$vN);
            }
          }
        }
      }
    ...
    if (Attributes.isFunInterface(kmClass)) {
      SPropertyOperations.assign(SNodeOperations.as(node, CONCEPTS.InterfaceDeclaration$fL), PROPS.isFunctional$pBP$, true);
    }

    if (Attributes.isExternal(kmClass) || Attributes.isExpect(kmClass)) {
    ...
    context.withTypeParameters(kmClass.getTypeParameters(), SNodeOperations.as(node, CONCEPTS.ITypeParameters$G$), fqName, (constraints, typeParameters) -> {
      // Add constraints
      ListSequence.fromList(SLinkOperations.getChildren(SNodeOperations.as(node, CONCEPTS.ITypeConstrainedParameters$KK), LINKS.constraints$BRhr)).addSequence(ListSequence.fromList(constraints));

      {
        final SNode inheriting = node;
      ...
      kmClass.getConstructors().forEach((cons) -> {
        ...
        if (SNodeOperations.isInstanceOf(res, CONCEPTS.PrimaryConstructor$QJ)) {
          SLinkOperations.setTarget(SNodeOperations.as(node, CONCEPTS.ClassDeclaration$Jm), LINKS.primaryConstructor$QvZc, SNodeOperations.cast(res, CONCEPTS.PrimaryConstructor$QJ));
        } else {
          ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.members$gqdV)).addElement(SNodeOperations.cast(res, CONCEPTS.SecondaryConstructor$Lg));
        }
      });

      // Declarations
      ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.members$gqdV)).addSequence(ListSequence.fromList(KtModuleParser.parseDeclarations(kmClass, fqName, context)));

      ListSequence.fromList(childClasses).visitAll((it) -> ListSequence.fromList(SLinkOperations.getChildren(node, LINKS.members$gqdV)).addElement(it));

      {
        final SNode enumClass = node;
        if (SNodeOperations.isInstanceOf(enumClass, CONCEPTS.EnumClassDeclaration$xK)) {
          final Set<String> names = SetSequence.fromSetWithValues(new HashSet<>(), ListSequence.fromList(enumEntries).select((it) -> {
            ListSequence.fromList(SLinkOperations.getChildren(enumClass, LINKS.entries$EB0i)).addElement(it);
            return SPropertyOperations.getString(it, PROPS.name$MnvL);
          }));

          // Add entries that were not added
          kmClass.getEnumEntries().forEach((name) -> {
            if (!(SetSequence.fromSet(names).contains(name))) {
              SNode entry = SLinkOperations.addNewChild(enumClass, LINKS.entries$EB0i, null);
  ...
  private static final class CONCEPTS {
    ...
    /*package*/ static final SConcept ValueClassModifier$84 = MetaAdapterFactory.getConcept(0x6b3888c1980244d8L, 0x8baff8e6c33ed689L, 0x72f0fe6919167bdaL, "jetbrains.mps.kotlin.structure.ValueClassModifier");
