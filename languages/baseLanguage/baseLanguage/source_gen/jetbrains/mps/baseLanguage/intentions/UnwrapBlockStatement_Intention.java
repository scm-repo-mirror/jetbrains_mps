...
public final class UnwrapBlockStatement_Intention extends AbstractIntentionDescriptor implements IntentionFactory {
  ...
  private boolean isApplicableToNode(final SNode node, final EditorContext editorContext) {
    return Sequence.fromIterable(AttributeOperations.getChildNodesAndAttributes(SLinkOperations.getTarget(node, LINKS.statements$uqR0), LINKS.statement$WHn8)).isNotEmpty();
  }
  private boolean isVisibleInChild(final SNode node, final SNode childNode, final EditorContext editorContext) {
    return !(SNodeOperations.hasRole(node, LINKS.ifFalseStatement$Xnu2)) && Objects.equals(SNodeOperations.getNodeAncestor(childNode, CONCEPTS.StatementList$TN, true, false), SLinkOperations.getTarget(node, LINKS.statements$uqR0));
  }
  @Override
  ...
  /*package*/ final class IntentionImplementation extends AbstractIntentionExecutable {
    ...
    public String getDescription(final SNode node, final EditorContext editorContext) {
      return (SNodeOperations.hasRole(node, LINKS.ifFalseStatement$Xnu2) ? "Unwrap the Else Body" : "Unwrap the Body of a BlockStatement");

    }
    @Override
    public void execute(final SNode node, final EditorContext editorContext) {
      if (SNodeOperations.hasRole(node, LINKS.ifFalseStatement$Xnu2)) {
