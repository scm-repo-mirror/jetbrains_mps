...
public class check_CorrectScope_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode thisExpression, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    if ((SLinkOperations.getTarget(thisExpression, LINKS.classConcept$Hbij) == null)) {
      return;
    }
    if (!(ListSequence.fromList(ThisExpression__BehaviorDescriptor.getPossibleClassifiers_idhGcjiYt.invoke(thisExpression)).contains(SLinkOperations.getTarget(thisExpression, LINKS.classConcept$Hbij)))) {
      {
        final MessageTarget errorTarget = new NodeMessageTarget();
        IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(thisExpression, "Can't access " + SPropertyOperations.getString(SLinkOperations.getTarget(thisExpression, LINKS.classConcept$Hbij), PROPS.name$tAp1), "r:00000000-0000-4000-0000-011c895902c5(jetbrains.mps.baseLanguage.typesystem)", "4539702814067113826", null, errorTarget);
      }
    }
  }
