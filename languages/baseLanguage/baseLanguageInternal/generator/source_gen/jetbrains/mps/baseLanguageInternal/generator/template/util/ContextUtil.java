  ...
  public static SNode getContextForConstant(TemplateQueryContext genContext, SNode node, boolean topmost) {
    ...
    if ((usage != null)) {
      if (topmost) {
        ...
        return ListSequence.fromList(SNodeOperations.getNodeAncestors(usage, CONCEPTS.ClassConcept$IY, false)).where(new IWhereFilter<SNode>() {
      ...
      SNode outclass = genContext.getCopiedOutputNodeForInputNode(SNodeOperations.getNodeAncestor(node, CONCEPTS.ClassConcept$IY, false, false));
      if ((outclass != null)) {
        return ListSequence.fromList(SNodeOperations.getNodeAncestors(outclass, CONCEPTS.ClassConcept$IY, true)).last();
      }
    }
    return null;
  }
