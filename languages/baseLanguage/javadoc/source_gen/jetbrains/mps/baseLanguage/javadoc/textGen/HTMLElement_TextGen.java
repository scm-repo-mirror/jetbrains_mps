...
public class HTMLElement_TextGen extends TextGenDescriptorBase {
  ...
  public void generateText(final TextGenContext ctx) {
    ...
    if (ListSequence.fromList(SLinkOperations.getChildren(ctx.getPrimaryInput(), LINKS.line$rPMv)).isEmpty()) {
      tgs.append("<");
      tgs.append(SPropertyOperations.getString(ctx.getPrimaryInput(), PROPS.name$rPM0));
      tgs.append(" />");
    } else {
      tgs.append("<");
      tgs.append(SPropertyOperations.getString(ctx.getPrimaryInput(), PROPS.name$rPM0));
      tgs.append(">");
      for (SNode item : SLinkOperations.getChildren(ctx.getPrimaryInput(), LINKS.line$rPMv)) {
        tgs.appendNode(item);
      }
      tgs.append("</");
      tgs.append(SPropertyOperations.getString(ctx.getPrimaryInput(), PROPS.name$rPM0));
