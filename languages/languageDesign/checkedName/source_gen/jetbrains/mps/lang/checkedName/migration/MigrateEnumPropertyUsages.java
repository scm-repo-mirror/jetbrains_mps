...
public class MigrateEnumPropertyUsages extends MigrationScriptBase {
  ...
  public void doExecute(final SModule m) {
    EnumUsagesMigration migration = new EnumUsagesMigration();
    {
      ...
      for (SNode node : CollectionSequence.fromCollection(CommandUtil.instances(CommandUtil.selectScope(null, context), CONCEPTS.PropertyRefExpression$8m, false))) {
        migration.migratePropertyReference(node, LINKS.propertyDeclaration$KMe2);
      }
    }
  }
  @Override
  public Iterable<Problem> check(SModule m) {
    {
      ...
      return CollectionSequence.fromCollection(CommandUtil.instances(CommandUtil.selectScope(null, context), CONCEPTS.PropertyRefExpression$8m, false)).where(new IWhereFilter<SNode>() {
        public boolean accept(SNode it) {
          return SNodeOperations.isInstanceOf(SNodeOperations.getParent(SLinkOperations.getTarget(it, LINKS.propertyDeclaration$KMe2)), CONCEPTS.EnumPropertyMigrationInfo$O3);
        }
        ...
        public UsageOfMigrateNodeNotMigratedProblem select(SNode it) {
          return new UsageOfMigrateNodeNotMigratedProblem(it, SLinkOperations.getTarget(it, LINKS.propertyDeclaration$KMe2));
        }
      });
    }
  }
