package util;

/*Generated by MPS */

import org.jetbrains.mps.openapi.model.SNode;
import jetbrains.mps.generator.template.TemplateQueryContext;
import org.jetbrains.annotations.NotNull;
import jetbrains.mps.lang.smodel.generator.smodelAdapter.SPropertyOperations;
import org.jetbrains.mps.openapi.language.SProperty;
import jetbrains.mps.smodel.adapter.structure.MetaAdapterFactory;

public final class NameHelper {
  private final SNode myRuleDecl;
  private final TemplateQueryContext myGenContext;

  public NameHelper(@NotNull SNode ruleDecl, TemplateQueryContext genContext) {
    myRuleDecl = ruleDecl;
    myGenContext = genContext;
  }

  @NotNull
  public String getGeneratedString() {
    return getGeneratedName() + "_id" + getGeneratedId();
  }

  @NotNull
  public String getGeneratedName() {
    if (SPropertyOperations.getString(myRuleDecl, PROPS.name$MnvL) == null) {
      myGenContext.showInformationMessage(myRuleDecl, "Name of the rule is null, creating a unique name");
    }
    return "check";
  }

  @NotNull
  public String getGeneratedId() {
    if (SPropertyOperations.getString(myRuleDecl, PROPS.ruleId$WGdw) == null) {
      myGenContext.showErrorMessage(myRuleDecl, "Id of the rule cannot be null");
    }
    return SPropertyOperations.getString(myRuleDecl, PROPS.ruleId$WGdw);
  }

  private static final class PROPS {
    /*package*/ static final SProperty name$MnvL = MetaAdapterFactory.getProperty(0xceab519525ea4f22L, 0x9b92103b95ca8c0cL, 0x110396eaaa4L, 0x110396ec041L, "name");
    /*package*/ static final SProperty ruleId$WGdw = MetaAdapterFactory.getProperty(0x47257bf378d3470bL, 0x89d98c3261a61d15L, 0x46263286dbf54aaL, 0x5d2e6079771f8cc0L, "ruleId");
  }
}
