    ...
    return new FilteringScope(superScope) {
      ...
      public boolean isExcluded(SNode node) {
        if (SNodeOperations.isInstanceOf(node, CONCEPTS.AbstractConceptDeclaration$UN)) {
          return !(((boolean) AbstractConceptDeclaration__BehaviorDescriptor.isSubconceptOf_id73yVtVlWOga.invoke(SNodeOperations.cast(node, CONCEPTS.AbstractConceptDeclaration$UN), targetConcept)));
        } else if (SNodeOperations.isInstanceOf(node, CONCEPTS.SubstituteMenu_Named$J)) {
          return !((boolean) AbstractConceptDeclaration__BehaviorDescriptor.isSubconceptOf_id73yVtVlWOga.invoke(IMenu__BehaviorDescriptor.getApplicableConcept_id1quYWAD18xk.invoke(SNodeOperations.cast(node, CONCEPTS.SubstituteMenu_Named$J)), targetConcept));
        }
        return false;
      }
    };
