...
public class EditorAssistantUtilities {
  ...
  public boolean hasFeatures() {
    return ListSequence.fromList(AbstractConceptDeclaration__BehaviorDescriptor.getPropertyDeclarations_idhEwILLM.invoke(SLinkOperations.getTarget(getEditorDeclaration(), LINKS.conceptDeclaration$sbgS))).any(new IWhereFilter<SNode>() {
    ...
    }) || ListSequence.fromList(AbstractConceptDeclaration__BehaviorDescriptor.getLinkDeclarations_idhEwILKK.invoke(SLinkOperations.getTarget(getEditorDeclaration(), LINKS.conceptDeclaration$sbgS))).any(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
  ...
  public <T> T getFirstCellValue(_FunctionTypes._return_P1_E0<? extends T, ? super SNode> valueCalculator) {
    ...
    while (QueueSequence.fromQueue(editorComponents).isNotEmpty()) {
      ...
      for (SNode nextCell : ListSequence.fromList(SNodeOperations.getNodeDescendants(SLinkOperations.getTarget(nextComponent, LINKS.cellModel$3wIV), CONCEPTS.EditorCellModel$5c, true, new SAbstractConcept[]{}))) {
        T value = valueCalculator.invoke(nextCell);
        if (value != null) {
          return value;
        }
      }
      QueueSequence.fromQueue(editorComponents).addSequence(ListSequence.fromList(SNodeOperations.getNodeDescendants(SLinkOperations.getTarget(nextComponent, LINKS.cellModel$3wIV), CONCEPTS.CellModel_Component$1X, true, new SAbstractConcept[]{})).select(new ISelector<SNode, SNode>() {
        public SNode select(SNode it) {
          return SLinkOperations.getTarget(it, LINKS.editorComponent$AnLw);
        }
      }));
    }
    return null;
