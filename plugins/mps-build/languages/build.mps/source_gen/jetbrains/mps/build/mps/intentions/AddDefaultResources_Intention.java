...
public final class AddDefaultResources_Intention extends AbstractIntentionDescriptor implements IntentionFactory {
  ...
  private boolean isApplicableToNode(final SNode node, final EditorContext editorContext) {
    return Sequence.fromIterable(SNodeOperations.ofConcept(SLinkOperations.getChildren(node, LINKS.sources$Pqd_), CONCEPTS.BuildMps_ModuleResources$lI)).isEmpty();
  }
  @Override
  ...
  /*package*/ final class IntentionImplementation extends AbstractIntentionExecutable {
    ...
    public void execute(final SNode node, final EditorContext editorContext) {
      SNode mr = SLinkOperations.addNewChild(node, LINKS.sources$Pqd_, CONCEPTS.BuildMps_ModuleResources$lI);
      SNode files = SLinkOperations.setNewChild(mr, LINKS.files$_9D0, null);
      // assume module descriptor file lives under a module dir as a sibling to resources/icons 
      SLinkOperations.setTarget(files, LINKS.dir$6hmv, BuildSourcePath__BehaviorDescriptor.getParent_id7wpYgMyTXsR.invoke(SLinkOperations.getTarget(node, LINKS.path$g7Oh)));
      SPropertyOperations.set(SLinkOperations.addNewChild(files, LINKS.selectors$6oar, CONCEPTS.BuildFileIncludesSelector$LC), PROPS.pattern$Odcv, "icons/**");
    }
    @Override
