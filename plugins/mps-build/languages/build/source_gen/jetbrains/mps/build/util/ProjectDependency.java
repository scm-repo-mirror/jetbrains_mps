...
public class ProjectDependency {
  ...
  public ProjectDependency collectDependencies() {
    ...
    ListSequence.fromList(myDependency).addSequence(ListSequence.fromList(dependencies).select(new ISelector<SNode, Tuples._2<SNode, String>>() {
      public Tuples._2<SNode, String> select(SNode it) {
        return MultiTuple.<SNode,String>from(SLinkOperations.getTarget(it, LINKS.script$UXIZ), calculatePath(it, helper));
      }
    }));
  ...
  private String calculatePath(SNode node, RelativePathHelper helper) {
    SNode script = SLinkOperations.getTarget(node, LINKS.script$UXIZ);
    String filePath = BuildProject__BehaviorDescriptor.getScriptsPath_id4ahc858UcHk.invoke(script, myContext);
    if (filePath == null) {
      myGenContext.showErrorMessage(script, "no script path for required script " + SPropertyOperations.getString(script, PROPS.name$lA7v));
      return ".";
    }
    try {
  ...
  private void dfs(SNode project, List<SNode> result, Set<SNode> visited) {
    ...
    for (SNode dependency : Sequence.fromIterable(getLocalDependencies(project))) {
      if (SetSequence.fromSet(visited).contains(SLinkOperations.getTarget(dependency, LINKS.script$UXIZ))) {
        continue;
      }
      dfs(SLinkOperations.getTarget(dependency, LINKS.script$UXIZ), result, visited);
      ListSequence.fromList(result).addElement(dependency);
    }
  }
  private Iterable<SNode> getLocalDependencies(SNode project) {
    return Sequence.fromIterable(SNodeOperations.ofConcept(SLinkOperations.getChildren(project, LINKS.dependencies$fxFr), CONCEPTS.BuildProjectDependency$Ug)).where(new IWhereFilter<SNode>() {
      public boolean accept(SNode it) {
        return (SLinkOperations.getTarget(it, LINKS.artifacts$BfmB) == null) && isProjectLocal(it);
      }
    });
  }
  ...
  private boolean isProjectLocal(SNode dep) {
    SModel targetScriptModel = SNodeOperations.getModel(SLinkOperations.getTarget(dep, LINKS.script$UXIZ));
