...
public class ItemConverter {
  ...
  private static ToolComponent.IItem convertActionItem(ActionItem actionItem, String folderName) {
    ...
    if (actionItem instanceof SidebarActionItem) {
      ...
      try {
        icon = GlobalIconManager.getInstance().getIconForResource(sidebarItem.getIcon());
      } catch (LinkageError error) {
        if (LOG.isWarningLevel()) {
          LOG.warning("Error trying to get icon for " + actionItem, error);
        }
      } catch (RuntimeException exception) {
        if (LOG.isWarningLevel()) {
          LOG.warning("Exception trying to get icon for " + actionItem, exception);
        }
      }
      ...
      try {
        ...
        if (LOG.isWarningLevel()) {
          LOG.warning("Error trying to get tooltip text for " + actionItem, error);
        }
      } catch (RuntimeException exception) {
        if (LOG.isWarningLevel()) {
          LOG.warning("Exception trying to get tooltip text for " + actionItem, exception);
        }
      }
    }
