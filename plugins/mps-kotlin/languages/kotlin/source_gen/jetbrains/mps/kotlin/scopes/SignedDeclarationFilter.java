package jetbrains.mps.kotlin.scopes;

/*Generated by MPS */

import jetbrains.mps.references.Reference;
import org.jetbrains.mps.openapi.language.SAbstractConcept;
import jetbrains.mps.kotlin.runtime.members.signature.MemberSignature;
import jetbrains.mps.kotlin.behavior.IIdentifier__BehaviorDescriptor;
import jetbrains.mps.lang.smodel.generator.smodelAdapter.SNodeOperations;

/**
 * Allow to filter from both concept and signature kind.
 * 
 * Filters are applied a two levels:
 * - signature filter is used to prevent creation of signature that do not matter during
 * processing (eg. ignore properties if working on functions, or functions whose name is different from some value...)
 * - concept filter is used to filter resulting nodes provided to scope (those could be useful during processing but are
 * omitted afterwards, eg. base language methods when getting scope for kotlin function call)
 */
public class SignedDeclarationFilter {
  private SignatureFilter mySignatureFilter;
  public SignatureFilter getSignatureFilter() {
    return this.mySignatureFilter;
  }
  private void _setSignatureFilter(SignatureFilter value) {
    this.mySignatureFilter = value;
  }
  private SignatureFilter setSignatureFilter(SignatureFilter value) {
    _setSignatureFilter(value);
    return value;
  }
  private Reference<SignatureFilter> refToSignatureFilter() {
    return new Reference<SignatureFilter>() {
      public SignatureFilter get() {
        return getSignatureFilter();
      }
      public void set(SignatureFilter value) {
        _setSignatureFilter(value);
      }
    };
  }
  private SAbstractConcept myConceptFilter;
  public SAbstractConcept getConceptFilter() {
    return this.myConceptFilter;
  }
  private void _setConceptFilter(SAbstractConcept value) {
    this.myConceptFilter = value;
  }
  private SAbstractConcept setConceptFilter(SAbstractConcept value) {
    _setConceptFilter(value);
    return value;
  }
  private Reference<SAbstractConcept> refToConceptFilter() {
    return new Reference<SAbstractConcept>() {
      public SAbstractConcept get() {
        return getConceptFilter();
      }
      public void set(SAbstractConcept value) {
        _setConceptFilter(value);
      }
    };
  }


  public SignedDeclarationFilter(SAbstractConcept filterConcept, SignatureFilter signatureFilter) {
    this.setConceptFilter(filterConcept);
    this.setSignatureFilter(signatureFilter);
  }

  public SignedDeclarationFilter(SAbstractConcept filterConcept, Class<? extends MemberSignature> signatureKind) {
    this(filterConcept, new SignatureFilter(signatureKind));
  }

  public SignedDeclarationFilter(SAbstractConcept filterConcept) {
    this(filterConcept, IIdentifier__BehaviorDescriptor.getMemberSignatureKind_id5q426iHFtTk.invoke(SNodeOperations.asSConcept(filterConcept)));
  }

  public DeclarationCollector toCollector() {
    return new DeclarationCollector(this);
  }
}
