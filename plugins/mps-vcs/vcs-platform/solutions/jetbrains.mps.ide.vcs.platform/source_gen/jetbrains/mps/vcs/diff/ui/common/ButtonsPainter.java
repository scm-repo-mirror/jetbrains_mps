...
public abstract class ButtonsPainter extends DiffFoldingAreaPainter {
  ...
  public int getLeftAreaWidth() {
    return (isHighlightLeft() ? 0 : myWidth);
  }
  @Override
  public int getRightAreaWidth() {
    return (isHighlightLeft() ? myWidth : 0);
  }
  @Override
  ...
  private void ensureButtonsCreated() {
    if (myButtons == null) {
      ...
      for (ChangeGroup cg : ListSequence.fromList(getChangeGroupLayout().getChangeGroups())) {
        int y = Math.max((int) cg.getBounds(isHighlightLeft()).start(), previousStart + GAP + ICON_SIZE);
  ...
  private static String check_of7u5x_a0a41(FoldingAreaButton checkedDotOperand) {
    if (null != checkedDotOperand) {
      return checkedDotOperand.getToolTipText();
    }
    return null;
  }
  private static void check_of7u5x_a0a0a51(FoldingAreaButton checkedDotOperand) {
    if (null != checkedDotOperand) {
