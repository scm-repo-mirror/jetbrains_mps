package jetbrains.mps.baseLanguage.methodReferences.test.tests;

/*Generated by MPS */

import org.junit.jupiter.api.Test;
import org.junit.Assert;
import jetbrains.mps.internal.collections.runtime.ISelector;
import java.util.function.Consumer;
import jetbrains.mps.baseLanguage.closures.runtime._FunctionTypes;
import java.util.List;
import jetbrains.mps.internal.collections.runtime.ListSequence;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.concurrent.atomic.AtomicInteger;
import jetbrains.mps.internal.collections.runtime.Sequence;
import java.util.stream.Stream;

public class MethodReferences_Test {
  @Test
  public void test_testReturnContext() throws Exception {
    Assert.assertEquals(returnContext().invoke(3), "3");

    SelectorProvider provider = () -> {
      return new ISelector<Integer, String>() {
        public String select(Integer obj) {
          return String.valueOf(obj);
        }
      };
    };
    Assert.assertEquals(provider.get().invoke(4), "4");

    Consumer<Integer> consumer = ((_FunctionTypes._return_P0_E0<Consumer<Integer>>) () -> Integer::bitCount).invoke();
    consumer.accept(3);

  }
  @Test
  public void test_testInstanceMethodCall() throws Exception {
    List<Integer> integers = ListSequence.fromListAndArray(new ArrayList<Integer>(), 0, 1, 0, 0, 1);
    final StringBuilder builder = new StringBuilder();
    // Here the instance is provided by the builder expression: (a) => builder.append(a)
    ListSequence.fromList(integers).visitAll((i) -> builder.append(i));
    Assert.assertEquals("01001", builder.toString());
  }
  @Test
  public void test_testStaticInstanceMethodCall() throws Exception {
    String[] content = new String[]{"A", "D", "C", "B"};
    // Here the instance is provided by the callee: (a, b) => a.compareToIgnoreCase(b)
    Arrays.sort(content, String::compareToIgnoreCase);
    Assert.assertEquals(content[1], "B");
  }
  @Test
  public void test_testConstructorInvocation() throws Exception {
    Builder<AtomicInteger, Integer> intBuider = AtomicInteger::new;
    Assert.assertEquals(intBuider.make(3).intValue(), 3);
  }
  @Test
  public void test_testStaticMethodCall() throws Exception {
    List<String> strings = ListSequence.fromListAndArray(new ArrayList<String>(), "3.9", "4.0", "7.9");
    Assert.assertEquals(ListSequence.fromList(strings).select((s) -> Double.parseDouble(s)).toList(), ListSequence.fromListAndArray(new ArrayList<Double>(), 3.9, 4.0, 7.9));
  }
  @Test
  public void test_testAbstractClassInterop() throws Exception {
    List<Integer> integers = ListSequence.fromListAndArray(new ArrayList<Integer>(), 0, 1, 0, 0, 1);
    final StringBuilder builder = new StringBuilder();
    // Method reference is here used to replace an abstract class
    forEachInList(integers, new AbstractVisited<Integer>() {
      public void visit(Integer i) {
        builder.append(i);
      }
    });
    Assert.assertEquals("01001", builder.toString());
  }
  @Test
  public void test_testCastContext() throws Exception {
    Object caller = (Builder<String, String>) String::new;
    Assert.assertEquals(((Builder<String, String>) caller).make("Test"), "Test");

    Object absCaller = (AbstractBuilder<String, String>) new AbstractBuilder<String, String>() {
      public String make(String original) {
        return new String(original);
      }
    };
    Assert.assertEquals(((AbstractBuilder<String, String>) absCaller).make("Test"), "Test");
  }
  @Test
  public void test_testConstructorContext() throws Exception {
    // Method reference in constructor call with generics
    AbstractVisited<String> visited = new AbstractVisited<>(new AbstractBuilder<String, String>() {
      public String make(String this0) {
        return this0.intern();
      }
    }) {
      @Override
      public void visit(String item) {
      }
    };
    visited.visit(null);
  }
  @Test
  public void test_testMethodParameters() throws Exception {
    List<StringBuilder> builders = ListSequence.fromListAndArray(new ArrayList<StringBuilder>(), new StringBuilder("Hello"), new StringBuilder("World"));
    // No method parameter specified (inferred)
    Assert.assertEquals(ListSequence.fromList(builders).select((item) -> Builder.copy(item)).first().toString(), "Hello");

    // With method parameter specified
    Assert.assertEquals(ListSequence.fromList(builders).select((item) -> Builder.<StringBuilder>copy(item)).last().toString(), "World");
  }
  @Test
  public void test_testChainedCalls() throws Exception {
    List<StringBuilder> builders = ListSequence.fromListAndArray(new ArrayList<StringBuilder>(), new StringBuilder("Hello"), new StringBuilder("World"));

    // Chained calls with inferred type from "builders"
    Iterable<String> strings = ListSequence.fromList(builders).select((content) -> ItemContainer.init(content)).select((this0) -> this0.get()).select((this0) -> this0.toString());
    Assert.assertEquals(Sequence.fromIterable(strings).first(), "Hello");
  }
  @Test
  public void test_testChainedCallsJavaType() throws Exception {
    List<StringBuilder> builders = ListSequence.fromListAndArray(new ArrayList<StringBuilder>(), new StringBuilder("Hello"), new StringBuilder("World"));

    // Chained calls with inferred type from "builders", using stream API
    Stream<String> map = builders.stream().map(ItemContainer::init).map(ItemContainer::self).map(ItemContainer::get).map(StringBuilder::toString);
    Assert.assertEquals(map.findFirst().get(), "Hello");
  }
  @Test
  public void test_testChainedCallsJavaFunctionType() throws Exception {
    List<StringBuilder> builders = ListSequence.fromListAndArray(new ArrayList<StringBuilder>(), new StringBuilder("Hello"), new StringBuilder("World"));

    // Chained calls with inferred type from "builders", using java implementation of sequences
    Iterable<String> strings = ListSequence.fromList(builders).select((content) -> ItemContainer.init(content)).select((this0) -> this0.self()).select((this0) -> this0.get()).select((this0) -> this0.toString());
    Assert.assertEquals(Sequence.fromIterable(strings).first(), "Hello");
  }
  @Test
  public void test_testChainedCallsFunctionType() throws Exception {
    Iterable<StringBuilder> builders = ListSequence.fromListAndArray(new ArrayList<StringBuilder>(), new StringBuilder("Hello"), new StringBuilder("World"));

    // Chained calls with inferred type from "builders", using sequences
    Iterable<String> strings = Sequence.fromIterable(builders).select((content) -> ItemContainer.init(content)).select((this0) -> this0.self()).select((this0) -> this0.get()).select((this0) -> this0.toString());
    Assert.assertEquals(Sequence.fromIterable(strings).first(), "Hello");
  }
  /*package*/ abstract class AbstractVisited<U> {
    public AbstractVisited() {
      // Constructor needed for generation to work if instanciated with method ref
    }
    public AbstractVisited(AbstractBuilder<U, U> b) {
    }
    /*package*/ abstract void visit(U item);
  }
  /*package*/ interface Builder<T, U> {
    T make(U capacity);
    static <M> M copy(M item) {
      return item;
    }
  }
  /*package*/ abstract class AbstractBuilder<T, U> {
    public abstract T make(U capacity);
  }
  /*package*/ <T> void forEachInList(List<T> items, final AbstractVisited<T> action) {
    ListSequence.fromList(items).visitAll((item) -> action.visit(item));
  }
  /*package*/ static class ItemContainer<E> {
    private E item;
    /*package*/ ItemContainer(E item) {
      this.item = item;
    }
    /*package*/ static <W> ItemContainer<W> init(W content) {
      return new ItemContainer(content);
    }
    /*package*/ E get() {
      return item;
    }
    /*package*/ ItemContainer<E> self() {
      return this;
    }
  }

  /*package*/ ISelector<Integer, String> returnContext() {
    if (System.currentTimeMillis() % 2 == 0) {
      return new ISelector<Integer, String>() {
        public String select(Integer obj) {
          return String.valueOf(obj);
        }
      };
    }
    return new ISelector<Integer, String>() {
      public String select(Integer obj) {
        return String.valueOf(obj);
      }
    };
  }

  public interface SelectorProvider {
    ISelector<Integer, String> get();
  }


}
