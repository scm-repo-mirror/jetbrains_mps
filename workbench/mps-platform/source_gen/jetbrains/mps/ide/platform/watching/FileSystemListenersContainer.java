...
public class FileSystemListenersContainer {
  ...
  private final ConcurrentMap<FileListenerAdapter, String> myListener2Path = new ConcurrentHashMap<>();
  ...
  public void addListener(@NotNull FileListener listener, @NotNull IFile file) {
    ...
    try {
      for (String s : normalizeAndSplit(path).collect(Collectors.toList())) {
        if ((s == null || s.isEmpty())) {
          continue;
        }
  ...
  public ListenersForPath getListenersForPath(String eventPath) {
    ...
    try {
      for (String s : normalizeAndSplit(eventPath).collect(Collectors.toList())) {
        if ((s == null || s.length() == 0)) {
          continue;
        }
  ...
  private Stream<String> normalizeAndSplit(@NotNull String path) {
    String normalized = FileUtil.normalize(path);
    String JAR_SEP = JarFileSystem.JAR_SEPARATOR;
    int jarRootIdx = normalized.indexOf(JAR_SEP);
    if (jarRootIdx < 0) {
      ...
      return Stream.of(prefix, JAR_SEP, suffix)
                   .flatMap(FileSystemListenersContainer::splitWithUnixSep);
    }
  }

  @NotNull
  private static Stream<String> splitWithUnixSep(String str) {
    return Stream.of(str.split(Path.UNIX_SEPARATOR));
  }

  /**
   * we store and issue FileListenerAdapters (from #getListenersFromPath)
   * but to be honest we have to check that the passed argument is not among delegates
   */
  public boolean tracks(@NotNull FileListener listener) {
    if (listener instanceof FileListenerAdapter) {
      return myListener2Path.containsKey(listener);
    }
    return myListener2Path.keySet()
                          .stream()
                          .anyMatch(it -> it.getListener().equals(listener));
  }
